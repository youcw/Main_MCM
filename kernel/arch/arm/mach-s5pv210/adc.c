...
static int resolution_adc;
static int prec;
static int delay_adc;
		...
		size_t size, loff_t *pos)
{
	...
	if(prec!=0)
	{
		writel((readl(base_addr + S3C_ADCCON))|S3C_ADCCON_PRSCEN | S3C_ADCCON_PRSCVL(prec & 0xff), base_addr + S3C_ADCCON);
	}
	if (delay_adc>0)
	{
		writel(delay_adc & 0xffff, base_addr + S3C_ADCDLY);
	}
	if(resolution_adc==12)
	{
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case ADC_RESOL:
		...
		{
			writel(readl(base_addr + S3C_ADCCON) | S3C_ADCCON_RESSEL_12BIT, base_addr + S3C_ADCCON);
			plat_data->resolution = 12;
			resolution_adc = 12;
		}
		else if(arg==10)
		{
			writel(readl(base_addr + S3C_ADCCON) & (~S3C_ADCCON_RESSEL_12BIT), base_addr + S3C_ADCCON);
			plat_data->resolution = 10;
			resolution_adc = 10;
		}
		return 0;
...
static int __devinit s3c_adc_probe(struct platform_device *pdev)
{
	...
	{
		delay_adc = plat_data->delay;
		writel(plat_data->delay & 0xffff, base_addr + S3C_ADCDLY);
	}
	if (plat_data->resolution == 12)
		{
			resolution_adc = plat_data->resolution;
