...
#include "s3c-dma.h"
//#undef  pr_debug
//#define pr_debug(fmt...) printk(fmt)//lhh add 

//#define tq_debug(fmt...) printk(fmt)//lhh add 
#define  tq_debug(fmt...) 
...
{
	tq_debug(KERN_DEBUG "%s: LRI=%d, TXFEMPT=%d, RXFEMPT=%d, TXFFULL=%d,\
		RXFFULL=%d\n", fn,
	...
	tq_debug(KERN_DEBUG "%s: PAUSE: TXDMA=%d, RXDMA=%d, TXCH=%d, RXCH=%d\n",
	       fn,
	       bit_set(con, S3C2412_IISCON_TXDMA_PAUSE),
	       bit_set(con, S3C2412_IISCON_RXDMA_PAUSE),
	       bit_set(con, S3C2412_IISCON_TXCH_PAUSE),
	       bit_set(con, S3C2412_IISCON_RXCH_PAUSE));
	tq_debug(KERN_DEBUG "%s: ACTIVE: TXDMA=%d, RXDMA=%d, IIS=%d\n", fn,
...
static void s3c2412_snd_rxctrl(struct s3c_i2sv2_info *i2s, int on)
{
	...
	if (on) {
		...
		default:
			dev_err(i2s->dev, "RXDIS: Invalid MODE l8n %x in IISMOD\n",
				mod & S3C2412_IISMOD_MODE_MASK);
		}
...
//			pr_debug("Entered %s,%d,%s,iiscon = 0x%x,readl(i2s->regs + S3C2412_IISMOD)= 0x%x\n",__FILE__,__LINE__,__func__,iiscon,readl(i2s->regs + S3C2412_IISMOD));
