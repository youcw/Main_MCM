	...
	unsigned short reg)
{
	...
	u32 stat, addr, data;
	clk_enable(s3c_ac97.ac97_clk);//一定要使能因为这个地方可能
				  ...
				  struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		dma_data = &s3c_ac97_pcm_out;
			...
			s3c_ac97_mic_in.dma_size = dma_tsfr_size;
	...
	temp = s3c_ac97_read(codec, 0x26);
	printk("s3c_ac97_read 0x26 :@@@@@@@@@@@@%x\n",temp);
	s3c_ac97_write(codec, 0x0c, 0x0808);
	temp = s3c_ac97_read(codec, 0x0c);
	printk("s3c_ac97_read 0x26 :**************%x\n",temp);
	s3c_ac97_write(codec,0x3c, 0xf803);
	s3c_ac97_write(codec,0x3e,0xb990);
	s3c_ac97_write(codec,0x62,0xda83);
	/*s3c_ac97_write(codec, AC97_MASTER, 0x8080);
	s3c_ac97_write(codec, AC97_HEADPHONE, 0x0606);
	s3c_ac97_write(codec, AC97_REC_GAIN, 0x00aa);*/
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		printk("\n\t4444444444444444CONFIG_SOUND_WM9713_INPUT_STREAM_MIC\n");
#else						/* Input Stream is LINE-IN : SMDKC110 default */
		s3c_ac97_write(codec, 0x14, 0xd612);
		writel(0x14d612, s3c_ac97.regs + S3C_AC97_CODEC_CMD);
#endif
	}
				...
				struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		...
		ac_glbctrl &= ~S3C_AC97_GLBCTRL_PCMOUTTM_MASK;//因为我们要用MIC访问所以不能这样做
				      ...
				      struct snd_soc_dai *dai)
{
	...
	else
	{
		...
		s3c_ac97_write(codec,0x62,0xda83);
		s3c_ac97_write(codec, 0x12, 0x0f0f);	/* Record Gain */
		//writel(0x120f0f, s3c_ac97.regs + S3C_AC97_CODEC_CMD);
#ifdef CONFIG_SOUND_WM9713_INPUT_STREAM_MIC	/* Input Stream is MIC-IN */
		s3c_ac97_write(codec,0x5c,0x2);
		s3c_ac97_write(codec,0x10,0x68);
		s3c_ac97_write(codec,0x14,0xfe00);
		/*writel(0x5c0002, s3c_ac97.regs + S3C_AC97_CODEC_CMD);
		writel(0x100068, s3c_ac97.regs + S3C_AC97_CODEC_CMD);
		writel(0x14fe00, s3c_ac97.regs + S3C_AC97_CODEC_CMD);*/
		printk("\n\t4444444444444444CONFIG_SOUND_WM9713_INPUT_STREAM_MIC\n");
#else						/* Input Stream is LINE-IN : SMDKC110 default */
		s3c_ac97_write(codec, 0x14, 0xd612);
		//writel(0x14d612, s3c_ac97.regs + S3C_AC97_CODEC_CMD);
...
static struct snd_soc_dai_ops s3c_ac97_dai_ops = {
	...
	.startup	= s3c_ac97_startup,	/* 添加内容 clock gating */
	.shutdown	= s3c_ac97_shutdown,	/* 添加内容 AC97 clock gating */
};
