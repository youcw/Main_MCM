...
ctrl->cap->flip |= (FIMC_XFLIP|FIMC_YFLIP);
#if 0
printk("Stream Format Informations:\n");
printk(" type: %d\n", f->type);
printk(" width: %d\n", f->fmt.pix.width);
printk(" height: %d\n", f->fmt.pix.height);
char fmtstr[8];
memset(fmtstr, 0, 4);
memcpy(fmtstr, &f->fmt.pix.pixelformat, 4);
...
log_msg(LOG_DBG,"V4L2_CID_IMAGE_EFFECT_APPLY(%d)\n",c->value);
...
log_msg(LOG_DBG,"V4L2_CID_IMAGE_EFFECT_FN(%d)\n",c->value);
...
log_msg(LOG_DBG,"V4L2_CID_IMAGE_EFFECT_CB(%d)\n",c->value);
		ctrl->fe.pat_cb = c->value & 0xFF;
		ret = 0;
		break;

	case V4L2_CID_IMAGE_EFFECT_CR:
log_msg(LOG_DBG,"V4L2_CID_IMAGE_EFFECT_CR(%d)\n",c->value);
		ctrl->fe.pat_cr = c->value & 0xFF;
		ret = 0;
		break;

	default:
log_msg(LOG_DBG,"default(id=%08x)(val=%d)\n",c->id,c->value);
		/* try on subdev */
		mutex_unlock(&ctrl->v4l2_lock);
		if (2 != ctrl->id)
...
//---------------------------------------
void print_regs(struct fimc_control *ctrl)
{
	...
	for(i=0;i<0x19c;i+=4)
	{
		regval = ioread32(ctrl->regs+i);
		printk("%04x_%04x == %04x,%04x ---",0,i,regval>>16,(unsigned short)regval&0xffff);
		for(j=31;j>=0;--j)
		{
			printk("%01x",(regval>>j)&0x00000001);
			if((j%4)==0)
				printk(",");
		}
		printk("\n");
...
#if 0
i=0x64;
iowrite32(0x020a0800,ctrl->regs+i);
regval = ioread32(ctrl->regs+i);
regval = ioread32(ctrl->regs+i);
printk("--------%04x_%04x == %04x,%04x ---",0,i,regval>>16,(unsigned short)regval&0xffff);
for(j=31;j>=0;--j)
