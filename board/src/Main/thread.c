...
int create_thread(void)
{
        ...
        ret = pthread_create(&thread_adc, NULL, thread_monitor_adc, NULL); 
        if (ret != 0){
                ERROR("create pthread for monitoring adc error\n");
                return ret;
        }
        
        ret = pthread_create(&thread_led, NULL, thread_monitor_led, NULL); 
        if (ret != 0){
                ERROR("create pthread for indicate system error\n");
                return ret;
        }
        return ret;
}
...
void *thread_monitor_adc(void *pdata)
{
        float vol;
...
void *thread_monitor_led(void *pdata)
{
        for (;;) {
                flash_led(0);
                sleep(1);
