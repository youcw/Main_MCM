...
/* Nand flash oob definition for 4Kb page size with 8_bit ECC */
static struct nand_ecclayout s3c_nand_oob_218 = {
        .useecc = MTD_NANDECC_AUTOPLACE,
        .eccbytes = 104, 
        .eccpos = {
		   24, 25, 26, 27, 28, 29, 30, 31,
...
static void s3c_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
	...
	if (nand_type == S3C_NAND_TYPE_SLC)
			nfconf |= /*(0<<23);//*/S3C_NFCONF_ECC_1BIT;
	else
			nfconf |= /*(2<<23);//*/S3C_NFCONF_ECC_4BIT;
#endif

	writel(nfconf, NFCONF);
//	writel(readl(NFCONF) | (1<<3), NFCONF);
...
#if defined(CONFIG_NAND_BL1_8BIT_ECC) && (defined(CONFIG_S5PC110) || defined(CONFIG_S5PV210))
/***************************************************************
...
void s3c_nand_enable_hwecc_8bit(struct mtd_info *mtd, int mode)
{
	...
	nfreg = readl(NFECCCONF);
	nfreg &= ~((0x3ff<<16));
	nfreg |= (0x1ff << 16);//config message lenth 511
	writel(nfreg, NFECCCONF);
	...
	/*writel(0, NFECCCONECC0);
	writel(0, NFECCCONECC1);
	writel(0, NFECCCONECC2);
	writel(0, NFECCCONECC3);
	writel(0, NFECCCONECC4);
	writel(0, NFECCCONECC5);
	writel(0, NFECCCONECC6);*/
}
...
int s3c_nand_read_page_8bit_for_irom(struct mtd_info *mtd, struct nand_chip *chip,
				uint8_t *buf)
{
...
//		puts("while(1) here! 844\n");
		s3c_nand_calculate_ecc_8bit(mtd, 0, 0);
//		puts("while(1) here! 846\n");
...
void board_nand_init(struct nand_chip *nand)
{
	...
	tq_temp = readb(nand->IO_ADDR_R);/*5tc*/
#if 0
	if(tq_temp &(0x1<<0))
	{
		puts("Ecc level.. 0\n");
...
//			puts("SLC......\n");
			...
			if (devID == 0xd5) {
				...
				//因为K9GAG08U0M和U0D的第四个ID定义不同，所以这里作调整。
				if ((tmp == 0x29) || (tmp == 0xB6))//((1024 << (tmp & 3)) == 4096)
				{
					/* Page size is 4Kbytes */
					nand->ecc.read_page = s3c_nand_read_page_8bit;
					nand->ecc.write_page = s3c_nand_write_page_8bit;
