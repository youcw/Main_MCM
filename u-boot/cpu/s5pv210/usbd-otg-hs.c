...
#if defined(CONFIG_S5PC110) || defined(CONFIG_S5PV210)
#include <command.h>
#include <asm/errno.h>
#include <asm/io.h>
#include <regs.h>
...
int s3c_usbctl_init(void)
{
	...
	writel(readl(USB_PHY_CONTROL)|(1<<0), USB_PHY_CONTROL);	/*USB PHY0 Enable */
	...
	case EP0_STATE_GD_STR_I1:
		...
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo((u8 *)string_desc1+(otg.ep0_substate*otg.ctrl_max_pktsize),
						sizeof(string_desc1)-(otg.ep0_substate*otg.ctrl_max_pktsize));
			otg.ep0_state = EP0_STATE_INIT;
			otg.ep0_substate = 0;
		}
		break;
...
#endif
			s3c_receive_done = 1;
			sprintf(buf, "%lx", (otg.dn_filesize-10) );
			setenv("filesize",buf);
...
#endif	/* CONFIG_S5PC110 || CONFIG_S5PV210 */
