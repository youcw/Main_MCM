...
#if defined(DEBUG_MTD)
#define debug_mtd(fmt,args...)	printf (fmt ,##args)
#define debugX_mtd(level,fmt,args...) if (DEBUG_MTD>=level) printf(fmt,##args);
#else
#define debug_mtd(fmt,args...)
#define debugX_mtd(level,fmt,args...)
#endif	/* DEBUG */
...
static void index_partitions(void)
{
	...
	if (current_mtd_dev) {
		...
		debug_mtd("=> mtddevnum %d,\n=> mtddevname %s\n", mtddevnum, part->name);
	} else {
		setenv("mtddevnum", NULL);
		setenv("mtddevname", NULL);

		debug_mtd("=> mtddevnum NULL\n=> mtddevname NULL\n");
	}
}
...
static void current_save(void)
{
	...
	if (current_mtd_dev) {
		...
		debug_mtd("=> partition %s\n", buf);
	} else {
		setenv("partition", NULL);
		last_partition[0] = '\0';

		debug_mtd("=> partition NULL\n");
	}
	index_partitions();
}
...
static int part_sort_add(struct mtd_device *dev, struct part_info *part)
{
	...
	if (list_empty(&dev->parts)) {
		debug_mtd("part_sort_add: list empty\n");
...
static int part_parse(const char *const partdef, const char **ret, struct part_info **retpart)
{
	...
	*ret = NULL;
	...
	if (*p == '-') {
		...
		debug_mtd("'-': remaining size assigned\n");
		size = SIZE_REMAINING;
		p++;
	} else {
	...
	debug_mtd("+ partition: name %-22s size 0x%08x offset 0x%08x mask flags %d\n",
			part->name, part->size,
...
static int device_parse(const char *const mtd_dev, const char **ret, struct mtd_device **retdev)
{
	...
	*retdev = NULL;
	...
	debug_mtd("dev type = %d (%s), dev num = %d, mtd-id = %s\n",
			id->type, MTD_DEV_TYPE(id->type),
			id->num, id->mtd_id);
	pend = strchr(p, ';');
	debug_mtd("parsing partitions %.*s\n", (pend ? pend - p : strlen(p)), p);
...
static struct mtdids* id_find_by_mtd_id(const char *mtd_id, unsigned int mtd_id_len)
{
	...
	debug_mtd("--- id_find_by_mtd_id: '%.*s' (len = %d)\n",
			mtd_id_len, mtd_id, mtd_id_len);

	list_for_each(entry, &mtdids) {
		id = list_entry(entry, struct mtdids, link);

		debug_mtd("entry: '%s' (len = %d)\n",
...
static int delete_partition(const char *id)
{
	...
	if (find_dev_and_part(id, &dev, &pnum, &part) == 0) {
		...
		debug_mtd("delete_partition: device = %s%d, partition %d = (%s) 0x%08x@0x%08x\n",
...
int mtdparts_init(void)
{
	...
	debug_mtd("last_ids  : %s\n", last_ids);
	debug_mtd("env_ids   : %s\n", ids);
	debug_mtd("last_parts: %s\n", last_parts);
	debug_mtd("env_parts : %s\n\n", parts);

	debug_mtd("last_partition : %s\n", last_partition);
	debug_mtd("env_partition  : %s\n", current_partition);

	/* if mtdids varible is empty try to use defaults */
	if (!ids) {
		if (mtdids_default) {
			debug_mtd("mtdids variable not defined, using default\n");
			ids = mtdids_default;
	...
	if ((last_ids[0] != '\0') && (strcmp(last_ids, ids) == 0)) {
		debug_mtd("check if last_ids[0] != '\0'\n");
		ids_changed = 0;
	} else {
		debug_mtd("check if last_ids[0] = = '\0'88888888\n");
		ids_changed = 1;
	...
	if (parts && ((last_parts[0] == '\0') || ((strcmp(last_parts, parts) != 0)) || ids_changed)) {
		...
		debug_mtd("mtdparts_init: current_mtd_dev  = %s%d, current_mtd_partnum = %d\n",
				MTD_DEV_TYPE(current_mtd_dev->id->type),
				current_mtd_dev->id->num, current_mtd_partnum);
	}
	...
	if ((tmp_ep[0] != '\0') && (strcmp(tmp_ep, last_partition) != 0)) {
		...
		debug_mtd("--- getting current partition: %s\n", tmp_ep);
		...
		debug_mtd("no partition variable set, setting...\n");
		current_save();
	}
