...
#endif
#ifdef  CONFIG_CONCENWIT_INPUT
static unsigned char input_val=0;
#endif
static __inline__ int abortboot(int bootdelay)
{
	int abort = 0;
	...
	if (bootdelay >= 0) {
		if (tstc()) {	/* we got a key press	*/
			//if(0x20 == getc())   /* consume input	*/
			if(getc())   /* consume input	*/
			{
				puts ("\b\b\b 0");
...
#if 1
#ifdef CONFIG_CONCENWIT_LCD
	//Lcd_printf(0,0,RGB(0,0,0xff),0,1,"Welcome using CW210 Board!");
#ifdef CONFIG_CW210_1G
	//Lcd_printf(0,16,RGB(0,0,0xff),0,1,"DRAM:1024MB,NAND:%dMB-%s.",nand_info[0].size/1024/1024,(nand_type == S3C_NAND_TYPE_SLC)?"SLC":"MLC");
#else
	//Lcd_printf(0,16,RGB(0,0,0xff),0,1,"DRAM:512MB,NAND:%dMB-%s.",nand_info[0].size/1024/1024,(nand_type == S3C_NAND_TYPE_SLC)?"SLC":"MLC");
#endif /* CONFIG_CW210_1G */
	//Lcd_printf(0,16*2,RGB(0,0,0xff),0,1,"LCD size:%dX%d", LCD_XSIZE_TFT, LCD_YSIZE_TFT);
	//concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,26*8,16*6,0xff,NULL);
/*	concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"111111111122233333312345678");
	concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"111111111122233333312345678");
	concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"111111111122233333312345678");
	concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"111111111122233333312345678");
	concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"111111111122233333312345678");
	...
	concenwit_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"111111111122233333312345678");
*/
	//Lcd_printf(0,16*3,RGB(0,0,0xff),0,1,"Hit any key to stop autoboot: %2d ", bootdelay);
...
#ifdef CONFIG_CONCENWIT_LCD
		//Lcd_printf(30*8,16*3,RGB(0,0,0xff),0,0,"%2d ", bootdelay);
#endif
	...
	putc('\n');
		...
		if(ret != 0xff){
			...
			concenwit_user_logo();
				...
				printf("Fat32 file system initialzation failed!\n");	
			}
		}
#endif
#ifdef CONFIG_SILENT_CONSOLE
...
/****************************************************************************/
#define rGPC1CON (*(volatile unsigned int *)0xE0200080)
#define rGPC1DAT (*(volatile unsigned int *)0xE0200084)
...
char buf1[6];
	...
	printf("\r\n##### %s BIOS for %s #####\r\n", buf1, buf2);
#ifdef CONFIG_CW210_1G
	printf("This Board: SDRAM is 1024MB; ");
#else
	printf("This Board: SDRAM is 512MB; ");
#endif
#ifdef CONFIG_CONCENWIT_LCD
	cw210_lcd_init();
	printf("LCD display size is: %dX%d\r\n", LCD_XSIZE_TFT, LCD_YSIZE_TFT);
#else
	printf("\n");
#endif
//rGPC1CON = (rGPC1CON &(~(0xff<<12))) | (0x11<<12);
//rGPC1DAT |= 0x3<<3;
