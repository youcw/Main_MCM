	...
	*maf_id = chip->read_byte(mtd);
	...
	if (!type->pagesize) {
		if (dev_id == 0xd5) {
			...
			/* Calc pagesize */
#ifdef CONFIG_CW210
			/* 因为K9GAG08UOM和K9GAG08U0D的第四个ID的定义不一样，所以这里直接填上4096和524288。 */
			if((extid == 0xB6) || (extid == 0x29))
			{
				mtd->writesize = 4096;//1024/*2048*/<< (extid & 0x3);
				mtd->erasesize = 524288;(/*128*/64 * 1024) << (extid & 0x03);
			}
			else//目前使用的是K9GAG08U0D的，屏蔽掉的是K9GAG08U0M。
			{
				mtd->writesize = 2048/*1024*/<< (extid & 0x3);
				mtd->erasesize = (128/*64*/ * 1024) << (extid & 0x03);
			}
			//extid >>= 2;
			/* Calc oobsize */
			if (((extid>>2) & 0x3) == 0x1) {
				mtd->oobsize = 128;
			} else if (((extid>>2) & 0x3) == 0x2) {
				mtd->oobsize = 218;
			}
			//extid >>= 2;
			//前面已经计算出来了ID了，所以这里就不再计算了。
			/* Calc blocksize. Blocksize is multiples of 64KiB */
			//mtd->erasesize = (/*128*/64 * 1024) << ((extid>>4) & 0x03);
			//extid >>= 2;
#else
