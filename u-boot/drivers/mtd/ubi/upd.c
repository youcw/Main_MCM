...
#ifdef CONFIG_OEM_SDREAD
int OEM_ubi_more_update_data(struct ubi_device *ubi, struct ubi_volume *vol,
			 const void __user *buf, int count, unsigned long Dir)
{
	uint64_t tmp;
	int lnum, offs, err = 0, len, to_write = count;
	int percent_complete = -1, length = count;
	 ...
	 */
	/*
	 * 下面的len的值根据Nand的类型不一样值不一样：2K页面的SLC是126976B，2K页面的MLC是258048B，4K页面的MLC是524288B。
	 */
	while (count)
	{
		...
		Dir = FAT_OEMReadFile(Dir, buf, len, 0);
		...
		lnum += 1;
		//buf += len;
		unsigned long long n = ((unsigned long long)(length - count) * 100);
		int percent;
		do_div(n, length);
		percent = (int)n;
		if (percent != percent_complete)
		{
			...
			printf("\rWriting %3d%% complete.", percent);
#ifdef CONFIG_CONCENWIT_LCD
			if(1 == percent)
			{
				concenwit_Write_Progress_note(0,0,0,1,0xff,"Download UBI File System.");
